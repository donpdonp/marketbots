...
class WarpBubble
  class MarketManager < Base
        ...
        if plan.state == "planning"
          ...
          if balances
            ...
            if purse > 0.001
              ...
              log "Go plan: plan cost #{"%0.8f"%plan.cost}. #{exg.name} #{"%0.8f"%purse}btc (trimmed #{"%0.8f"%purse_trim})available.", "beacon"
    ...
    def email_complete(payload)
      ...
      if plan.state == "email"
        ...
        if payload["exchange"] == from_exg.name
          log("Buying #{from_exg.name} transfer confirmed","beacon")
          plan.state = "moved"
          set('warpbubble:plan', plan.to_simple)
        else
      ...
      if plan.state == "bought"
        ...
        if balances
          ...
          if balance > plan.purse
            ...
            log "post-plan profit #{plan.purse-plan.spent}btc - #{fee}%fee = #{(plan.purse-plan.spent)*(1-fee)}btc", "beacon"
            plan.state = "sold"
            set('warpbubble:plan', plan.to_simple)
          else
    ...
    def place_orders(plan, purse, fee)
      ...
      original_purse = plan.purse
      plan.purse = plan.purse*(1-fee)
      log("#{state} orders finished. totals #{expense}btc #{acquired}ltc. plan.purse = #{plan.purse} (original purse #{original_purse}). plan.spent = #{plan.spent}")
    end
