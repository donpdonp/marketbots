...
var max_speed = 0
var big_button = true
var highwater = config.quant.highwater
...
mtgoxob.on('trade', function(trade){
  if(trade.price_currency == 'USD') {
    ...
    msg += '$'+trade.price.toFixed(2)+
           ' x'+trade.amount.toFixed(1)+' '

    msg += "highwater "+highwater+" "
    var highprice = highwater * (1-0.05)
    msg += "highprice "+highprice.toFixed(2)+" "
    if(trade.price < highprice) { 
      if(big_button){
        sell(trade.price) 
        big_button = false
      } else {
        json_log({alert:"Sell "+trade.price+" blocked by big button"})
      }
    }

    msg += "fixed $"+config.quant.fixed_price+" x"+config.quant.fixed_quantity+" "

    var pricediff = ((trade.price-config.quant.fixed_price)/config.quant.fixed_price)*100
    msg += 'diff '+pricediff.toFixed(1)+'% '
    ...
    if(trade.price > highwater) { set_highwater(trade.price) }
...
function set_highwater(price){
  json_log({msg:"new highwater "+price})
  highwater = price
}
